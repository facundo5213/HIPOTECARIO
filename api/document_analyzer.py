...
def scan_update_sheet(table: dict, regulation: str, effective_date: date, existing_products: set, existing_equivalences: dict) -> list:
   ...
   for row_index, row in enumerate(table["rows"], start=1):
       ...
       if not nuevo_consolidacion and not consolidacion_actual:
           ...
           # sólo se registra el alta del producto de desembolso nuevo, ya hecha arriba.
           continue
...
def analyze(content: bytes, extension: str, document_id: int, effective_date: date, existing_organisms=None, existing_products=None, existing_equivalences=None):
   ...
   for table_index, table in enumerate(source["tables"], start=1):
       ...
       if sheet_reg:
           findings.extend(scan_update_sheet(table, sheet_reg, effective_date, existing_products, existing_equivalences))
